...
public partial class EFDataModel : DbContext
{
    ...
    // => optionsBuilder.UseSqlite("Data Source=C:\\Working\\PJProcessor.db");
    // => optionsBuilder.UseMySQL("Server=localhost;Database=PJProcessor;User=admin;Password=admin");
    // => optionsBuilder.UseNpgsql("Host=localhost;Database=PJProcessor;Username=postgres;Password=admin");
    // => optionsBuilder.UseSqlServer("Data Source=(local);Initial Catalog=PJProcessor;Persist Security Info=True;User ID=sa;Password=saPassword;MultipleActiveResultSets=True;TrustServerCertificate=True;");
